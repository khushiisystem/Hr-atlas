...
export class Tab1Page implements OnInit, AfterViewInit, OnDestroy {
    ...
    this.userStateServ.getState().subscribe((res) => {
      if (res) {
        ...
        if (res.role === "Employee") {
          ...
          this.isSwitchable = false;
          if (
            this.userDetails.currentAddress.addressLine1 == null ||
            this.userDetails.currentAddress.addressLine1.trim() == "" ||
  ...
  getTodayAttendance(dateStr: string, pageIndex: number) {
    ...
    this.apiSubscription = this.adminServ
      .getDailyAttendance(dateStr, pageIndex * 5, 5)
      .subscribe(
        (res) => {
          if (res.length < 1) {
  ...
  getAttendance() {
    this.apiSubscription = this.shareServ.todayAttendance().subscribe(
      (res) => {
        if (res) {
          let hours = 0;
          let minutes = 0;
          ...
          for (const workTime of res) {
            if (workTime.workingTime) {
              const [hours, minutes] = workTime.workingTime
                .split(":")
                .map(Number);
  ...
  getCalendar() {
    this.eventsList = [];
    this.apiSubscription = this.adminServ
      .getEventHollyday(moment.utc(this.today).format())
      .subscribe(
        (res) => {
          if (res) {
            if (res.length < 1) {
              ...
              this.eventsList = res;
              const sortArray = this.eventsList.sort(
                (a, b) =>
                  new Date(a.eventDate).getTime() -
                  new Date(b.eventDate).getTime()
              );
              const filteredArray = sortArray.filter(
                (item) => new Date(item.eventDate) >= new Date(this.today)
              );
  ...
  fetchBirthdays() {
    ...
    this.apiSubscription = this.shareServ.upcomingBirthday().subscribe(
      (res) => {
        if (!res || res.length < 1) {
  ...
  getTodaysBirthday() {
    ...
    const todaysBirthdays = this.birthdayList.filter((item) =>
      getTime(item.dateOfBirth)
    );
  ...
  mathcDate(dateStr: string | Date, dateStr2: string | Date) {
    return (
      moment(dateStr).year() === moment(dateStr2).year() &&
      moment(dateStr).month() === moment(dateStr2).month() &&
      moment(dateStr).date() === moment(dateStr2).date()
    );
  }

  clockIn() {
    this.loader.present("");
    this.shareServ.clockIn().subscribe(
      (res) => {
        if (res) {
          this.isRunning = true;
          this.buttonLabel = "Clock Out\t";
          const currentTime = new Date(
            moment(res.clockIn).format()
          ).toISOString();
  ...
  waitUntil(condition: () => boolean, interval = 100) {
  ...
  async clockOut() {
    this.timesheetTimeFlag = false;
    this.todayWorkingTimeFlag = false;
    this.timesheetTime = 0;

    this.getTimesheetTimeOfDay();
    this.getTodayAttendanceTime();

    this.loader.present("");
    await this.waitUntil(
      () => this.timesheetTimeFlag && this.todayWorkingTimeFlag
    );
    this.loader.dismiss();
        ...
        buttons: [
          ...
          },
          {
            ...
            handler: () => {
              ...
              if (this.clockInId) {
                this.shareServ.clockOut(this.clockInId).subscribe(
                  (res) => {
                    if (res) {
                      this.isRunning = false;
  ...
  calculateDurationAndAddSeconds(
    ...
    const formattedResult = `${hours.toString().padStart(2, "0")}:${minutes
      .toString()
  ...
  getName() {
    if (this.userDetails.lastName && this.userDetails.lastName.trim() !== "") {
      return `${this.userDetails.firstName.slice(
        0,
        1
      )}${this.userDetails.lastName.slice(0, 1)}`;
    } else {
  ...
  getLleaveSetup() {
    this.apiSubscription = this.adminServ.getLeaveSetup().subscribe(
      (res) => {
        if (res) {
          this.leaveDone = true;
  ...
  getAttendanceSetups() {
    this.apiSubscription = this.adminServ.getAttendanceSetup().subscribe(
      async (res) => {
        if (!res) {
    ...
    this.adminServ.leaveApprove(leaveData).subscribe(
      (res) => {
        if (res) {
          ...
          this.loader.dismiss();
          const requestIndex = this.requestedLeaveList.findIndex(
            (item) => item.guid === event.leaveId
          );
  ...
  getTodayAttendanceTime() {
    this.timesheetSer.getTodayAttendanceTime().subscribe((res) => {
      this.todayWorkingTime = res;
      this.todayWorkingTimeFlag = true;
    });
  }
}
