...
export class SalarySetupPage implements OnInit {
  ...
  setEffectiveDate(event: DatetimeCustomEvent) {
    this.salarySetupForm.patchValue({
      effectiveDate: moment(event.detail.value).startOf('day').utc().format(),
    });
  ...
  getPreviousRevision() {
    ...
    this.adminServ.getEmloyeeReviseSalary(this.employeeId).subscribe(
      (res) => {
        if (res) {
          this.salarySetupForm.patchValue({
  ...
  calculateTotal() {
    ...
    if (this.previousSalary > 0 && currentCtc >= 0) {
      const percent =
        ((currentCtc - this.previousSalary) / this.previousSalary) * 100;
      this.salarySetupForm.patchValue({
  ...
  submit() {
    if (this.salarySetupForm.invalid) {
      ...
      this.adminServ.employeeSalarySetup(this.salarySetupForm.value).subscribe(
        (res) => {
          if (res) {
            this.shareServ.presentToast(
              "Salary setup successfully",
              "top",
              "success"
            );
            this.isInProcess = false;
